 ...
 * @idea: 判断一个链表是否为回文结构 给定一个链表的头节点head，请判断该链表是否为回文结构。 例如： 1->2->1，返回true。
 *        1->2->2->1，返回true。 15->6->15，返回true。 1->2->3，返回false。 【要求】
 *        如果链表长度为N，时间复杂度达到O(N)，额外空间复杂度达到O(1) 快慢指针找到中间位置， 逆序后半截， 比较 ， 再次逆序后半截
 * @author Huaigui
 */
class Node {
	public int value;
...
public class IsPalindromeList {
	 ...
	 * @Title: isPalindrome1 
	 * @idea:
	 * 栈的方法判断:遍历一次，将所有的节点存储到栈里，第二次遍历是栈一次弹出比较 
	 * @param head 
	 */
