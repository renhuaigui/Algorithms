 ...
 * @idea: 关于如何查找数组中和最大的子序列 问题：给定一个数组，如{-1,3,6,-9,2,-5,-1,9,3,-3}，求该数组中的和最大的子序列。
 *        思路：最简单的方法，以每个元素作为起点，再考虑1~n所有的子序列元素个数的情况，这样的复杂度就为O(n*n).
 *        之后我们讨论了如何简化数组，比如相邻的正数可以将其绑定，两端的负数可以直接排除，最后数组可以被简化为两端为正数，
 *        中间没有相邻正数的形式。然而，这样并没有从根本上降低算法的复杂度， 分治策略的复杂度为O(nlogn), 动态规划算法的复杂度为O(n).
 * @author Huaigui
 * @date 2016年4月19日 下午7:09:29
 * 
 */
public class MaxSubArray {
	...
	/**
	 * 先从第一个元素开始向后累加， 每次累加后与之前的和比较. 
	 * 保留最大值， 再从第二个元素开始向后累加，以此类推。
	 */
	...
	public int[] DivideConquer(int[] a, int left, int right) {
		/**
		 * 分治法O(NlgN) 1、完全位于左边的数组中。 2、完全位于右边的数组中。 3、跨越中点，包含左右数组中靠近中点的部分。
	...
	public int[] MaxSubSum(int[] a) {
		/**
		 * 动态规划法： b[i]保存以a[i]结尾的最大子序列的和，
		 * 则b[i]=max(b[i-1]+a[i],a[i]),但是对于b[i]只有b[i-1]>0时b[i]=b[i-1]+a[i] else
		 * b[i]=a[i] 重新开始累加
		 */
		int[] result = new int[3];
		int maxsum = a[0];
