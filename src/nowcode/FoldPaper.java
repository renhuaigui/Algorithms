 ...
 *       给定折的次数n,请返回从上到下的折痕的数组，若为下折痕则对应元素为"down",若为上折痕则为"up". 测试样例： 1
 *       返回：["down"] 2 返回：["down","down","up"]
 * @author Huaigui
 * @date 2016年4月27日 下午10:38:59
 * 
 */
public class FoldPaper {
	...
	public String[] foldPaper(int n) {
		/**
		 * 首先，第一次折叠后，序列是d。 第二次折叠后，序列是ddu。 第三次折叠后，序列是ddudduu。
		 * 经过分析发现，每次的序列，是在前一次的每个间隔中，以dudu。。。的顺序插入的。 比如：第一次的是d，它有左右两个空位，分别插入d
		 * u后就变为了ddu。 第二次是ddu，从第一个d左边到第三个位置的u右边，共有四个空位，分别插入dudu后，结果是ddudduu
		 */
		// write code here
