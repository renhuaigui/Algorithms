 ...
 * 题目： 给定一个没有重复元素的数组A，定义A上的MaxTree如下：MaxTree的根节点为A中最大的数，
 * 根节点的左子树为数组中最大数左边部分的MaxTree，右子树为数组中最大数右边部分的MaxTree。请根据给定的数组A，
 * 设计一个算法构造这个数组的MaxTree。
 * 
 * 解答： 如果能够确定每个节点的父亲节点，则可以构造出整棵树。找出每个数往左数第一个比他大的数和往右数第一个比他大的数，
 ...
 * 
 * 面试官角度： 首先容易想到的是使用递归的方法来构造MaxTree，每一层递归用O(n)的时间找到最大数，然后将数组分为左右两个部分，
...
public class MaxTree {
	...
	/**
	 * @Title: stackBuild @Description:用栈的方法解 从左到右遍历每个数，栈中保持递减序列，
	 *         新来的数不停的Pop出栈顶直到栈顶比新数大或没有数 @throws
	 */
	...
	/**
	 * @Title: violenceBuild @Description: 暴力解法（O(n^2)） @throws
	 */
