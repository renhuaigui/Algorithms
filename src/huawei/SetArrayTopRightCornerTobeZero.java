...
* @Description: 数组右上三角置0
描述:	
定义了3×3的二维数组，并在主函数中自动赋值。请编写函数SetArrayTopRightCornerTobeZero(int a[][3])，该函数的功能是：使数组右上半三角元素中的值全部置成0（对角线元素也为0，特别注意：由于矩阵显示的特殊性，此处输入/输出格式，具体格式见样例）。
...
输出:	
右上半三角元素中的值全部置成0的3×3二维数组（特别注意：由于矩阵显示的特殊性，此处输入/输出格式，具体格式见样例）
