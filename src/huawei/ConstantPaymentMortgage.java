...
* @Description: 等额本息还贷
描述:	
目前，银行的个人住房货款的还款方式主要有等额本息和等额本金两种。
等额本息每月按相同金额还贷款本息，月还款中利息逐月递减，本金逐月递增；
等额本金每月还款金额递减，月还款中本金保持相同数额，利息逐月递减。 
...
如上所述等额本息还款方式，在月供“本金与利息”的分配比例中，前面所还的利息比例大、本金比例小，后面逐步转为本金比例大、利息比例小，我们的任务就是找出这值得纪念的时刻。
运行时间限制:	无限制
内存限制:	无限制
输入:	
可以一行或多行，
每行首先是贷款金额（单位万元），然后是贷款年限（单位年，一年十二个月，按月还款，每月为一期），最后是年贷款利率（单位%，精确到小数点后两位，取值为0.00~100.00）。
注意所谓多行输入，类似于将一个多行文件作为输入，例如内容如下：
20 10 9.99
200 200 9.99
输出:	
与输入行一一对应，
每行中首先输出原输入行中的贷款金额、贷款年限和年贷款利率，
然后输出第几个还款期次（即第几个月）开始，偿还的本金大于偿还的利息，
如果不存在，输出0。
注意多行输入对应多行输出，类似于将一个多行文件作为输入，然后输出一个多行文件，例如如上输入样例，输出内容如下：
20 10 9.99 38
200 200 9.99 2400
样例输入:	
20 20 5.04
