...
public class SimplePassword {
	public static String encryption(String str) {
		...
		for (int i = 0; i < oldStr.length; i++) {
			if (oldStr[i] >= 'a' && oldStr[i] <= 'c')
				oldStr[i] = '2';
			if (oldStr[i] >= 'd' && oldStr[i] <= 'f')
				oldStr[i] = '3';
			if (oldStr[i] >= 'g' && oldStr[i] <= 'i')
				oldStr[i] = '4';
			if (oldStr[i] >= 'j' && oldStr[i] <= 'l')
				oldStr[i] = '5';
			if (oldStr[i] >= 'm' && oldStr[i] <= 'o')
				oldStr[i] = '6';
			if (oldStr[i] >= 'p' && oldStr[i] <= 's')
				oldStr[i] = '7';
			if (oldStr[i] >= 't' && oldStr[i] <= 'v')
				oldStr[i] = '8';
			if (oldStr[i] >= 'w' && oldStr[i] <= 'z')
				oldStr[i] = '9';
			if (oldStr[i] >= 'A' && oldStr[i] <= 'Z')
				oldStr[i] = (char) ((oldStr[i] + 32 + 1 - 'a') % 26 + 'a');
		}
