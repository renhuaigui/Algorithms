...
public class TraversalBinaryTree {
	...
	private static String PreorderVisitRecur(Node head) {
		if (head == null)
			...
			return "" + head.val + PreorderVisitRecur(head.left) + PreorderVisitRecur(head.right);
		}
	}

	/**
	 * 前序遍历 非递归方式 根据前序遍历访问的顺序，优先访问根结点，然后再分别访问左孩子和右孩子。
	 * 即对于任一结点，其可看做是根结点，因此可以直接访问，访问完之后，若其左孩子不为空，
	 * 按相同规则访问它的左子树；当访问其左子树时，再访问它的右子树。因此其处理过程如下： 对于任一结点P：
	 * 
	 * 1)访问结点P，并将结点P入栈;
	 * 
	...
	private static String MidOrderVisitRecur(Node head) {
		if (head == null)
			...
			return "" + MidOrderVisitRecur(head.left) + head.val + MidOrderVisitRecur(head.right);
		}
	}

	/**
	 * 中序序遍历 非递归方式 根据中序遍历的顺序，对于任一结点，优先访问其左孩子，而左孩子结点又可以看做一根结点，
	 * 然后继续访问其左孩子结点，直到遇到左孩子结点为空的结点才进行访问， 然后按相同的规则访问其右子树。因此其处理过程如下： 对于任一结点P，
	 * 
	...
	private static String PostOrderVisitRecur(Node head) {
		if (head == null)
			...
			return "" + PostOrderVisitRecur(head.left) + PostOrderVisitRecur(head.right) + head.val;
		}
	}

	/**
	 * 后序遍历 非递归方式 后序遍历的非递归实现是三种遍历方式中最难的一种。
