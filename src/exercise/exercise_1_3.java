...
public class exercise_1_3 {
	...
	/**
	 * 算法4 练习1.3.9 编写一个程序，从标准输入得到一个缺少左括号的表达式并打印出补全括号之后的中序表达式。 例如，给定输入：
	 * 1+2)*3-4)*5-6))) 程序应该输出 ((1+2)*((3-4)*(5-6))) 输入的表达式只考虑标准情况 每个操作符都对应一个右括号
	 */
	...
	public int Properties(String s) { // 比较运算符的大小（优先级）
	...
	/**
	 * 练习1.3.10 实现算术表达式由中序转为后序表达式(只能接受正确的表达式[字符串的格式])
	 */
	public String InfixToPostfix(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			} else if (str.charAt(i) == ')') { // 如果是’）‘则把运算符栈中的所有“（”加到表达式中
				String opout = "";
			...
			if (!op.empty())
				System.out.println("栈顶： " + op.peek());
		}
		while (!op.empty()) { // 弹出其余的符号
			System.out.println("出栈：" + op.peek());
			exp = exp + op.pop();
		}
		return exp;
