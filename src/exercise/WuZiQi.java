...
/**
 * @ClassName: WuZiQi
 * @idea: （粤港澳补充题）五子棋 描述: 此题目限定的五子棋规则： 1、棋盘大小：15*15
 *        2、分为黑棋和白棋，一种颜色棋子不论横、竖、斜连成5个或大于5个算赢 3、黑白交替下，也就是说一方不能连续2次成功落子。
 *        要求：根据双方落子，进行有效性判断和胜负判断。 运行时间限制: 无限制 内存限制: 无限制 输入:
 *        输入一行或多行，每行包括三个整数，分别是棋子类型，棋子的横坐标，棋子的纵坐标，以空格隔开 棋子类型：0 黑棋，1 白棋
 *        棋子坐标从0开始，小于15。 输出: 针对每行输入，如果： 黑方胜，输出1； 白方胜，输出2； 落子失败，比如坐标错误，或顺序错，输出-1；
 *        其他情况：落字成功，不输出； 胜负一旦确定，测试用例就结束。 样例输入: 0 7 7 1 7 8 0 7 7 1 7 5 0 8 8 1 9
 *        9 0 6 6 1 8 9 0 5 5 1 7 9 0 4 4 样例输出: -1 -1 1
 * @author Huaigui
 */
public class WuZiQi {
	/*
	...
	public static void main(String[] args) {
		...
		/*
		 * System.out.println(b[0]); System.out.println(b[1]);
		...
		while (!over & sc.hasNext()) {
			...
			b[2] = Integer.parseInt(s[2]);
			/*
			 * System.out.println(flag); System.out.println(b[0]);
			 * System.out.println(b[1]); System.out.println(b[2]);
			 */
			if (flag != b[0] && b[1] < 15 & b[1] > -1 && b[2] < 15 && b[2] > -1 && a[b[1]][b[2]] == 2) {
				flag = b[0];
				a[b[1]][b[2]] = b[0];
				if (winRow(a, b[1], b[2]) || winColumn(a, b[1], b[2]) || winRightDown(a, b[1], b[2])
						|| winLeftDown(a, b[1], b[2])) {
					over = true;
					if (b[0] == 0)
