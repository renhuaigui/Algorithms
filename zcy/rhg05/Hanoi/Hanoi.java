...
public class Hanoi {
	...
	public static int process(int num, String left, String mid, String right, String from, String to) {
		...
		if (from.equals(mid) || to.equals(mid)) {//两种有一个是mid. ,即不是夸中间操作
			String another = (from.equals(left) || to.equals(left)) ? right : left;// 俩个中一个是mid或者left ,则另一个是right ,f否则是left
	...
	/** 
	* @Title: fStackTotStack 
	* @Description: 
	* @param record  记录上一个动作
	* @param preNoAct  上一个不能做的动作
	* @param nowAct    当前动作
	* @param fStack   从哪个找操作
	* @param tStack   到哪个栈
	* @param from  打印用的从哪个栈
